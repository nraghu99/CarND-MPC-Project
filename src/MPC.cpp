 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // the Solver function below.
      ...
      // The part of the cost based on the reference state. i.e. cte, angle and velocity
      for (int t = 0; t < N; t++) {
          fg[0] += cte_cost_weight * CppAD::pow(vars[cte_start + t], 2);
          fg[0] += epsi_cost_weight * CppAD::pow(vars[epsi_start + t], 2);
          // This so that we have a minimum velocity
          fg[0] += CppAD::pow(vars[v_start + t] - ref_v, 2);
      }
      
      // Minimize the use of actuators.
      // this is for a smoother ride, no wobbling and no acc + deec
      for (int t = 0; t < N - 1; t++) {
          fg[0] += CppAD::pow(vars[delta_start + t], 2);
          fg[0] += CppAD::pow(vars[a_start + t], 2);
      }
      
      // Minimize the value gap between sequential actuations.  keep change rate low
      // Provide a smoother turn and spped control between time stamps
      for (int t = 0; t < N - 2; t++) {
      ...
      for (int t = 1; t < N; t++) {
          ...
          // Here we are subtracting as the steering angle is opposite in the controller
